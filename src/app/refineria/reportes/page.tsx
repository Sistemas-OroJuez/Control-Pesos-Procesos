...
export default function ReporteFinalAuditoria() {
  ...
  // --- MANTENEMOS TU REPORTE ORIGINAL ---
  ...
  // --- NUEVA VISTA: REPORTE GERENCIAL (LOGICA EXCEL) ---
  const fetchBalanceGerencial = async () => {
    setLoading(true);
    try {
      // Usamos la vista inteligente que creamos en SQL
    ...
    <div className="min-h-screen bg-black text-white p-4 font-sans uppercase text-[10px]">
      ...
      <div className="bg-zinc-900 rounded-[35px] border border-white/5 overflow-hidden shadow-2xl min-h-[400px]">
        {loading ? (
          ...
          /* TABLA DE AUDITORIA ORIGINAL (Tal cual la tenías) */
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-white/5 text-zinc-500 border-b border-white/10 font-black">
                  ...
                  <th className="p-4 text-right text-[9px]">L. ACTUAL</th>
              ...
              <tbody>
                {registros.map((r, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-all">
                    ...
                    <td className="p-4 text-center">{r.foto_url && <a href={r.foto_url} target="_blank" className="bg-zinc-800 p-2 rounded-lg">📸</a>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          /* TABLA BALANCE GERENCIAL (ESTILO EXCEL) */
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-blue-900/20 text-blue-400 border-b border-blue-500/10 font-black text-[8px]">
                  ...
                  <th className="p-3 text-right">AGL PROD.</th>
                  <th className="p-3 text-right">REPROCESO</th>
                  <th className="p-3 text-right">INV. DS3 FINAL</th>
              ...
              <tbody className="text-[10px]">
                {balanceData.map((b, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-blue-500/5 transition-all">
                    ...
                    <td className="p-3 text-right tabular-nums">{b.total_cpo?.toLocaleString()}</td>
                    <td className="p-3 text-right tabular-nums text-emerald-400">{b.total_rbd?.toLocaleString()}</td>
                    <td className="p-3 text-right tabular-nums text-orange-400">{b.agl_produccion?.toLocaleString()}</td>
                    <td className="p-3 text-right tabular-nums text-zinc-500">{b.reproceso?.toLocaleString() || 0}</td>
                    <td className="p-3 text-right tabular-nums text-blue-400">{b.inv_final_ds3?.toLocaleString()}</td>
                    <td className="p-3 text-right tabular-nums font-black text-white">{b.balance_acp?.toLocaleString()}</td>
                    <td className="p-3 text-right tabular-nums text-red-400">{b.diferencia_kg?.toLocaleString()}</td>
