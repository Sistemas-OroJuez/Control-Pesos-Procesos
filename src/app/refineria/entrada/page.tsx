...
export default function IngresoACP() {
  ...
  const handleCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    img.onload = async () => {
      ...
      // 1. PRE-PROCESAMIENTO: Filtro para texto blanco sobre fondo negro
      ctx.filter = 'grayscale(100%) contrast(300%)';
      ctx.drawImage(img, 0, 0);

      // 2. UMBRALIZACIÓN E INVERSIÓN (Threshold manual)
      const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
      const data = imageData.data;
      for (let i = 0; i < data.length; i += 4) {
        const avg = (data[i] + data[i + 1] + data[i + 2]) / 3;
        // Si el pixel es claro (texto), lo ponemos negro (0). Si es oscuro, blanco (255).
        const color = avg > 130 ? 0 : 255; 
      ...
      try {
        ...
        // 3. OCR con configuración de solo números y letras de ID
        const result = await Tesseract.recognize(processedURL, 'eng');
        const textOCR = result.data.text.toUpperCase();
        
        // 4. BÚSQUEDA DE ID EN CATÁLOGO (Limpiando caracteres extraños)
        ...
        if (!equipoVinculado) {
          alert("❌ ERROR: No se detectó el ID del equipo (Fila superior). Intente tomar la foto más cerca del panel.");
          setLoading(false);
          return;
        }

        // 5. EXTRACCIÓN POR PATRONES NUMÉRICOS
  ...
  return (
    <div className="min-h-screen bg-black p-4">
      ...
      <div className="max-w-md mx-auto bg-[#1a1a1a] rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/5">
        ...
        <div className={`p-8 text-white text-center ${esReproceso ? 'bg-red-600' : 'bg-blue-700'}`}>
          <h1 className="text-2xl font-black italic uppercase tracking-tighter italic">Ingreso ACP</h1>
          <p className="text-[10px] font-bold opacity-50 tracking-[0.2em] mt-1 uppercase">Control de Flujo</p>
        </div>
        ...
        <div className="p-6 space-y-6">
          <div className="text-center">
            {fotoUrl ? (
              <div className="relative">
                <img src={fotoUrl} className="w-full rounded-3xl border border-blue-500/30 shadow-lg" alt="Procesada" />
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={loading} className="w-full h-48 border-2 border-dashed border-white/10 rounded-[2rem] flex flex-col items-center justify-center text-gray-500 hover:border-blue-500 transition-all bg-white/5">
                <span className="text-5xl mb-3">{loading ? '⌛' : '📸'}</span>
                <span className="text-[11px] font-black uppercase tracking-widest">{loading ? 'Analizando Fila por Fila...' : 'Escanear Lector'}</span>
              </button>
            )}
          ...
          {datosConfirmados && (
            <div className="bg-[#050505] p-6 rounded-[2rem] border border-white/5 space-y-5 font-mono shadow-inner">
              ...
              <div className="text-center">
                <p className="text-[9px] text-gray-600 font-black uppercase tracking-[0.2em] mb-2">Totalizador Σ1</p>
          ...
          <textarea 
            ...
            placeholder="Anotaciones de la operación..."
          />
