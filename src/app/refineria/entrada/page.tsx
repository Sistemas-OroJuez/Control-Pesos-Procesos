...
export default function IngresoACP() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`seguimiento-ingreso-${ticketId}`)
      .on('postgres_changes', { 
          ...
          if (payload.new.status === 'completado') {
            ...
            alert("Error IA Entrada: " + payload.new.ia_raw);
            setLoading(false);
          }
      }).subscribe();
  ...
  const handleCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Nombre de archivo identificado como ingreso
      ...
      // Creamos el ticket en lecturas_ia para seguimiento
      ...
      alert("Error en Captura: " + err.message);
      setLoading(false);
    }
  };
  ...
  const handleConfirmarYGuardar = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .insert([{
          tipo_operacion: 'INGRESO_ACP', // Identificador de entrada
          valor_lectura: parseFloat(datos.totalizador), 
          foto_url: fotoUrl,
          observaciones: observaciones,
          masa_kg_h: 0, // Opcional: extraer del ia_raw si es necesario
          temperatura_c: parseFloat(datos.temperatura),
          densidad_kg_l: 0.8900, // Densidad promedio ACP
          es_reproceso: false, 
      ...
      alert("✅ INGRESO DE ACP REGISTRADO CORRECTAMENTE");
      // Limpiar estados
      setDatos(null);
      setFotoUrl(null);
      setObservaciones('');
        ...
        {!datos ? (
          ...
          <div className="flex flex-col items-center border-2 border-dashed border-zinc-800 rounded-[40px] p-10 bg-zinc-900/20">
            <button 
              ...
              className={`w-32 h-32 rounded-full flex items-center justify-center transition-all ${
                loading ? 'bg-zinc-900 animate-pulse' : 'bg-blue-600 shadow-2xl shadow-blue-900/40 active:scale-90'
              }`}
            >
              {loading ? (
            ...
            <p className="mt-8 text-zinc-600 text-[11px] font-black text-center tracking-widest leading-tight">
              {loading ? "IA ANALIZANDO ENTRADA..." : "CAPTURAR ENTRADA ACP"}
            </p>
          </div>
        ) : (
          /* RESULTADOS DE IA */
          <div className="bg-zinc-900 rounded-[40px] p-8 border border-white/5 space-y-6 animate-in zoom-in shadow-2xl">
            ...
            <textarea 
                ...
                className="w-full bg-black/40 rounded-2xl p-4 text-[10px] text-white border border-white/5 focus:ring-1 focus:ring-blue-500"
                placeholder="NOTAS DE INGRESO (EJ. PROVEEDOR, PLACA)..."
            />
            ...
            <button 
                ...
                disabled={loading}
                className="w-full py-6 bg-blue-600 rounded-2xl font-black text-xs tracking-[0.2em] shadow-lg active:scale-95 disabled:bg-zinc-700"
            >
                {loading ? "GUARDANDO REGISTRO..." : "CONFIRMAR Y REGISTRAR ENTRADA"}
            </button>
          </div>
        )}
