...
export default function Dashboard() {
  ...
  // 1. Empezamos en null para que no asuma ningún rol por error
    ...
    async function checkUser() {
      try {
        ...
        if (session?.user) {
          ...
          // 2. VALIDACIÓN ESTRICTA POR CORREO
          // Si es el correo del operador, rol 'operador'. Cualquier otro de la empresa es 'administrador'.
  ...
  const modules = allModules.filter(mod => {
    // Si todavía está cargando, no mostramos nada por seguridad
    if (loading) return false;
    // Si es administrador, ve todo
    if (userRole === 'administrador') return true;
    // Si es operador, solo ve los que NO son adminOnly
  ...
  return (
    <div className="min-h-screen bg-gray-100">
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div>
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase mt-1">
              {loading ? 'VALIDANDO ACCESO...' : `ACCESO AUTORIZADO: ${userRole?.toUpperCase()}`}
            </p>
          </div>
          ...
          <button 
            ...
            className="bg-gray-800 hover:bg-black text-white px-5 py-2 rounded-lg font-bold text-xs shadow-lg"
          >
            🚪 SALIR
          </button>
        </div>
      ...
      <footer className="fixed bottom-0 w-full p-4 text-center bg-gray-100/80 backdrop-blur-sm">
        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-[0.3em]">OroJuez S.A. - Control de Pesos</p>
      </footer>
    </div>
  );
}